...
#POP FORECASTS
pop_proj_path_utla = 'build_data\\pop_projections\\pop_forecast_24_to_43.csv'
pop_proj_path_district = 'build_data\\pop_projections\\district_pop_forecast_24_to_43.csv'
...
with st.expander('Assumptions and caveats'): 
    ...
    st.write('3. 2024 prevalence is applied to future years unless indicated. This is because. while we know prevalence will change for risk factors / conditions, we do not have this information available for all risk factors in a standard form/for each forecast year.')
...
if use_case == 'For just one service':
    ...
    with col1:
        ...
        if geography_level == 'Upper Tier or Unitary Authority':
            ...
            list_options = list_possible_utlas
        ...
        #TODO
            ...
            list_options = list_possible_district_las
    ...
    if apply_known_prevalence_to_service == 'Yes':
        with col1:
            model_prevalence = st.slider(label='Model prevalence of for service demand / condition', min_value=0.1, max_value=100.0, step=0.1)
    with col2:
        model_modifiable_risk_factor = st.selectbox(label='Would you like to also quantify prevalence of modifiable risk factor in your service demand (E.g. number of smokers etc.)?', options=['Yes', 'No'], index=1)
        if model_modifiable_risk_factor == 'Yes':
            with col2:
                mod_risk_factor_prevalence = st.slider(label='Model prevalence of for modifiable risk factor', min_value=0.1, max_value=100.0, step=0.1)

    try:
        st.subheader('Population change in selected areas:')
        
        test_df = forecast_population(
        pop_df,
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.dataframe(test_df.iloc[:,1:])
        with col2:
            st.altair_chart(create_population_change_chart(test_df, chart_metric))
